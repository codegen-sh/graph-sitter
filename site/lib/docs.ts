...
export function getDocsPage(parts?: string[]): DocsPage | null {
  ...
  if (filePath) {
    ...
    const title = readString(parsed.data.title) ?? titleize(slug.split("/").at(-1) ?? slug);
...
function getMintConfig() {
  if (!mintConfigCache) {
    mintConfigCache = JSON.parse(fs.readFileSync(mintPath, "utf8")) as MintConfig;
  }
...
function getDocsMeta(slug: string, section?: string): DocsMeta {
  ...
  const raw = fs.readFileSync(filePath, "utf8");
  const parsed = matter(raw);
  const title = readString(parsed.data.title) ?? titleize(normalized.split("/").at(-1) ?? normalized);
...
function getFileSlugs() {
  ...
  fileSlugCache = walk(docsRoot)
    ...
    .map((file) =>
      normalizeDocSlug(path.relative(docsRoot, file).replace(/\.(md|mdx)$/u, ""))
    )
...
function excerpt(content: string) {
  ...
  return plain ? `${plain.slice(0, 180)}${plain.length > 180 ? "..." : ""}` : undefined;
}
