...
function Card({
  ...
  const content = (
    <>
      {img ? (
        <img alt="" src={img} className="aspect-video w-full rounded-lg object-cover" />
      ) : null}
      {title ? (
...
function Return({
  ...
  return (
    <section className="my-6 rounded-xl border border-border bg-card p-5">
      <h4 className="mt-0 mb-3 text-sm font-semibold text-foreground">Returns</h4>
...
export const mdxComponents = {
  ...
  ChevronRight,
  Code2,
  Component: ({ children }: ChildrenProps) => <code>{children ?? "Component"}</code>,
  Github: ({ children }: ChildrenProps) => <code>{children ?? "GitHub"}</code>,
  Instance: ({ children }: ChildrenProps) => <code>{children ?? "Instance"}</code>,
  MyComponent: ({ children }: ChildrenProps) => <code>{children ?? "MyComponent"}</code>,
  New: ({ children }: ChildrenProps) => <code>{children ?? "New"}</code>,
  NewFeature: ({ children }: ChildrenProps) => <code>{children ?? "NewFeature"}</code>,
  NewUI: ({ children }: ChildrenProps) => <code>{children ?? "NewUI"}</code>,
  Old: ({ children }: ChildrenProps) => <code>{children ?? "Old"}</code>,
  OldFeature: ({ children }: ChildrenProps) => <code>{children ?? "OldFeature"}</code>,
