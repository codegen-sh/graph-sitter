...
- `TypeScriptIndex.promise_chains_json()`, `promise_chains_for_file_json(file_id)`, `promise_chains_for_symbol_json(symbol_id)`, and `promise_chain_by_id_json(chain_id)` expose compact read-only Promise-chain records without requiring full index JSON deserialization.
