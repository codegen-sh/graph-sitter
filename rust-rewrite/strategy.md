...
- [x] Add graph debug dump for nodes, edges, and usage metadata. owner: codex. Result: `PythonIndex` and `TypeScriptIndex` now expose `debug_graph_json()` through PyO3 and the Python Rust backend, with normalized file/symbol/import/export nodes plus containment, resolution, reference, dependency, external-reference, subclass, and export edges carrying compact record IDs.
...
- [ ] Expand Python/Rust parity fixtures to cover full dependency semantics, imported-export chains, and mutation flows.
