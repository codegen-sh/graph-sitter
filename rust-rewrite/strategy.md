...
- [x] Add parity tests for rename/move/add-import flows on Rust backend. owner: codex. Result: Python and TypeScript `Codemod.execute` parity tests now run identical add-import/rename and move-to-file import-update flows against Python and compact Rust backends; add-import/rename checks compare exact file bytes, while move checks compare nonblank line sequences because current backends differ only in blank-line preservation.
- [ ] Decide and enforce exact whitespace parity for move-to-file codemods. Notes: compact Rust move output currently matches Python backend on significant lines and import retargeting but preserves fewer blank lines around removed/moved declarations.
