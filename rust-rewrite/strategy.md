...
- [ ] Add Rust-backed codemod mutation smoke tests for graph-aware symbol/import flows. Notes: remaining parity slices should assert `function.rename`, `file.add_import` plus decorator edit, `create_file` plus `move_to_file`, and remove flows against focused Python fixtures before trying the full codemod harness.
