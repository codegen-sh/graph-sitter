...
- [x] Add pinned large TypeScript/JavaScript compact snapshot proof. owner: codex. Result: `snapshot_pinned_typescript_repo.py` and `test_pinned_nextjs_snapshot.py` verify Next.js `v15.0.0` at `51bfe3c1863b191f4b039bc230e8ed5c57b0baf3`; committed compact hashes/samples cover 13,688 selected files, 23,957 symbols, 28,210 imports, and 16,026 exports before TypeScript resolver/dependency parity.
...
- [x] 2026-06-18: Added opt-in pinned Next.js Rust `Codebase` compatibility/performance check. owner: codex. Notes: the new checker validates 13,688 compact files, 23,957 symbols, 28,210 imports, 16,026 exports, 37,554 references, 15,176 dependencies, blocked Python graph access, and conservative wall/RSS ceilings for the Python-facing TypeScript shell.
...
- [x] 2026-06-19: Added first compact TypeScript reference/dependency graph slice. owner: codex. Notes: Rust TS/JS indexing now emits and exposes reference/dependency rows through PyO3, snapshots them in the small TS golden and pinned Next.js proof, and resolves direct imported symbol uses, namespace member uses, and same-file top-level uses. Refreshed pinned Next.js `v15.0.0` benchmark: Rust emits 37,554 TS references and 15,176 dependency edges while staying 5.434x faster with 10.439x lower max RSS than Python parse/object materialization.
- [x] 2026-06-19: Wired the TypeScript compact index into the Python Codebase shell. owner: codex. Notes: strict Rust backend mode now accepts TypeScript codebases, calls the PyO3 `index_typescript_path(s)` APIs, returns compact TS files/symbols/interfaces/types/imports, and serves dependency/usages from Rust records while keeping the Python graph blocked. Measured pinned Next.js `v15.0.0` through `Codebase(..., language="typescript", graph_backend="rust")`: 13,688 compact files, 23,957 symbols, 37,554 references, 15,176 dependencies, 4.742s wall, and 362 MB max RSS.
